...
@interface IQTextFieldSupport : NSObject<UITextFieldDelegate>
...
@implementation IQTextFieldSupport
...
- (void)textFieldDidEndEditing:(UITextField *)textField{
    ...
    if(validateOnResign)
        [(IQTextField *)textField validate];
}

- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    [(IQTextField *)textField dismissPopup];
    if(validateOnCharacterChanged)
        [(IQTextField *)textField performSelector:@selector(validate) withObject:nil afterDelay:0.1];
    else
        [(IQTextField *)textField setRightView:nil];
...
@interface IQTextField(){
    NSString *strLengthValidationMsg;
    IQTextFieldSupport *supportObj;
    NSString *strMsg;
...
-(id)initWithCoder:(NSCoder *)aDecoder{
    ...
    supportObj=[[IQTextFieldSupport alloc] init];
...
-(void)addConfirmValidationTo:(IQTextField *)txtConfirm withMsg:(NSString *)msg{
...
-(BOOL)validate{
    ...
    for (int i=0; i<[arrRegx count]; i++) {
        ...
        if([dic objectForKey:@"confirm"]){
            IQTextField *txtConfirm=[dic objectForKey:@"confirm"];
